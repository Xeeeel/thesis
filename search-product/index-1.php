...
// Fetch products
$query = "SELECT * FROM products"; // Change 'products' to your actual table name
    ...
    if (isset($result) && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
              <div class="card border-0 shadow-sm rounded-3 product-card">
                <div class="position-relative">
                  <img src="/cartsy/' . htmlspecialchars($row["image_path"]) . '" class="card-img-top product-img" alt="Product Image">
                  <button class="btn btn-outline-danger position-absolute top-0 end-0 m-2 save-btn">
                    <i class="bi bi-heart"></i>
                  </button>
                </div>
                <div class="card-body text-center">
                  <h6 class="card-title fw-bold">' . htmlspecialchars($row["name"]) . '</h6>
                  <p class="text-muted small mb-1">Condition: ' . htmlspecialchars($row["condition_name"]) . '</p>
                  <p class="text-danger fw-bold fs-5">₱' . htmlspecialchars($row["price"]) . '</p>
                  <button class="btn btn-dark w-100">View Details</button>
                </div>
              </div>
            </div>';
        }
    } else {
        echo '<p class="text-muted text-center">No products found.</p>';
    }
    ?>
          </div>
        </section>
