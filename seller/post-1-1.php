...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert data into the database
$sql = "INSERT INTO products (product_name, price, category, condition_name, location, description, seller_id, image_path) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("sdssssi", $title, $price, $category, $condition_status, $location, $description, $user_id, $image_path);

if ($stmt->execute()) {
    $product_id = $stmt->insert_id; // Get last inserted ID
    // Do not close the statement here; we'll close it after the image upload section
    ...
    // Close the statement after all database operations are done
    $stmt->close();

    echo "<script>alert('Product added successfully!');</script>";
} else {
    echo "<script>alert('Error: " . $stmt->error . "');</script>";
    // Close the statement in case of an error as well
    $stmt->close();
}
