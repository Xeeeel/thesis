...
$seller_image = "https://via.placeholder.com/50";  // Default placeholder image
...
if (isset($_SESSION["id"])) {
    ...
    if ($stmt->num_rows > 0) {
        $stmt->fetch();
        $seller_name = htmlspecialchars($name);
        // Check if the profile picture exists and append the path
        $seller_image = !empty($profile_picture) ? "http://localhost/cartsy/profile/" . htmlspecialchars($profile_picture) : "https://via.placeholder.com/50";
    }
    $stmt->close();
}

// Handle form submission to add a new item for sale
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $deal_option = htmlspecialchars(trim($_POST['deal_option']));
    $description = htmlspecialchars(trim($_POST['description']));

    // Simple validation to check if all fields are filled
    if (!empty($title) && !empty($price) && !empty($category) && !empty($condition) && !empty($location) && !empty($deal_option) && !empty($description)) {
        
        // Insert item data into the database
        $stmt = $conn->prepare("INSERT INTO items (seller_id, title, price, category, condition, location, deal_option, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssss", $user_id, $title, $price, $category, $condition, $location, $deal_option, $description);

        if ($stmt->execute()) {
            echo "<div class='alert alert-success'>Item listed successfully!</div>";
        } else {
            echo "<div class='alert alert-danger'>Error listing item. Please try again later.</div>";
        }
        $stmt->close();
    } else {
            ...
            <div class="col-lg-4">
                <div class="sidebar">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <img src="<?= $seller_image ?>" class="rounded-circle me-2" alt="User" width="50">
                        <div><strong><?= $seller_name ?></strong><br><small>Listing to Cartsy</small></div>
                    </div>
