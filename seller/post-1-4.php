...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Correct the SQL query to match the number of parameters
$sql = "INSERT INTO products (product_name, price, category, condition_name, location, description, seller_id, thumbnail, deal) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);

// Correctly bind all 9 parameters, including the thumbnail (which is an empty string initially)
$stmt->bind_param("sdsssssss", $title, $price, $category, $condition_status, $location, $description, $seller_id, $thumbnail, $deal);
if ($stmt->execute()) {
        ...
        echo "<script>alert('Product added successfully!'); window.location.href='http://localhost/cartsy/seller/test-8.php';</script>";
    } else {
