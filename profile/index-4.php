...
if (!isset($_SESSION['id'])) {
    header("Location: http://localhost/cartsy/login/login.php"); // Redirect users who are not logged in
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_SESSION['id'])) {
        if ($password) {
            ...
            $stmt = $conn->prepare($query);
            $stmt->bind_param("ssssssissi", $email, $username, $name, $gender, $birth_month, $birth_day, $birth_year, $phone_number, $address, $_SESSION['id']);
        }
    } else {
...
<div class="profile-container shadow">
    <div class="sidebar position-relative">
        ...
        <a href="http://localhost/cartsy/uploads/logout.php"><i class="bi bi-box-arrow-right logout-icon"></i></a>
    </div>
